...
function saveUser(firstname, lastname, fullname, email, userid, profilepicture, moodleRoom) {
    ...
    client.query(`INSERT INTO moodledatauser(firstname, lastname, fullname, email, userid, profilepicture, moodleRoom) SELECT 'firstName', 'lastname','fullname','email','userid','profilepicture','moodleRoom', WHERE NOT EXISTS (SELECT * FROM moodledatauser WHERE $5 = userid))`, (err, res) => {
        if (err) {
