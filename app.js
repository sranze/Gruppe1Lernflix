...
const {
  messageFormatter,
  welcomeMessage
} = require('./backend/messages'); // make messages.js available
const {
  userJoin,
  getCurrentUser,
  userLeave
} = require('./backend/users'); // make functions in users.js available
const {
  saveUser,
  loadRooms,
  saveRooms,
  saveFeedback,
  loadFeedback
} = require('./backend/database'); // make database functions available
const {
  loadVideoInformation,
  saveVideoInformation
} = require('./backend/videos'); // make room (video-information) functions available
const {
  loadFlags,
  saveFlag,
  removeFlag
} = require('./backend/flags') // make flag functionalities available
...
app.post("*", require("body-parser").urlencoded({
  extended: true
}));

var moodleFirstName, moodleLastName, moodleFullName, moodleEmail, moodleUserID, moodleProfilePicture, moodleRoom, user;
  ...
  if (typeof moodleFirstName === 'undefined' || moodleFirstName == 'undefined' || moodleFirstName == undefined || moodleFirstName == null) {
    ...
    socket.on('removeFlag', flagInformation => {
      ...
      if (typeof flagInformation.videoURL !== 'undefined' && typeof flagInformation.videoTime !== 'undefined' && typeof user !== 'undefined') {
        ...
        var isSuccess = {
          message: "Ein Fehler ist beim Entfernen der Flagge aufgetreten.",
          isSuccess: false
        };
        io.to(socket.id).emit('error', isSuccess);
      }
